...
TEST_F(TestSimpleExtentManager, allocateExtentAndMerge) {
    ...
    ASSERT_EQ(kMinAllocSize, toAllocExtents.front().len);
    ...
    ASSERT_EQ(kMinAllocSize, extents.volumeextents(0).length());
    ASSERT_EQ(false, extents.volumeextents(0).isused());

    for (int i = 0; i < kMinAllocSize/len; i++) {
    ...
    ASSERT_EQ(pOffset + kMinAllocSize, toAllocExtents.front().pOffsetLeft);
    ASSERT_EQ(false, toAllocExtents.front().rightHintAvailable);

    std::list<Extent> allocatedExtents2;
    Extent ext2;
    ext2.set_offset(pOffset + kMinAllocSize);
    ext2.set_length(kMinAllocSize);
    ...
    ASSERT_EQ(0, extents.volumeextents(0).fsoffset());
    ASSERT_EQ(pOffset, extents.volumeextents(0).volumeoffset());
    ASSERT_EQ(kMinAllocSize * 2, extents.volumeextents(0).length());
    ASSERT_EQ(false, extents.volumeextents(0).isused());
}
...
TEST_F(TestSimpleExtentManager, MarkExtentsWritten) {
    ...
    for (int i = 1; i < kMinAllocSize/len; i++) {
        ...
        ASSERT_EQ(kMinAllocSize - i * len, extents.volumeextents(1).length());
