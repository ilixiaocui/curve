...
CURVEFS_ERROR SimpleExtentManager::GetToAllocExtents(const VolumeExtentList &extents,
    uint64_t offset,
    uint64_t len,
    ...
    if (len > 0) {
        // prealloc
        if (len > minAllocSize_) {
            len = (len / minAllocSize_ + 1) * minAllocSize_;
        } else {
            len = minAllocSize_;
        }
        ExtentAllocInfo allocInfo;
...
CURVEFS_ERROR SimpleExtentManager::DivideExtents(const VolumeExtentList &extents,
    uint64_t offset,
    uint64_t len,
