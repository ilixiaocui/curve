...
void InitMdsOption(Configuration *conf, MdsOption *mdsOpt) {
    LOG_IF(FATAL, !conf->GetStringValue(
        "mds.mdsaddr", &mdsOpt->mdsaddr));
    LOG_IF(FATAL, !conf->GetUInt64Value(
        "mds.rpcTimeoutMs", &mdsOpt->rpcTimeoutMs));
}

void InitMetaServerOption(Configuration *conf,
    MetaServerOption *metaOpt) {
    LOG_IF(FATAL, !conf->GetStringValue(
        "metaserver.msaddr", &metaOpt->msaddr));
    LOG_IF(FATAL, !conf->GetUInt64Value(
        "metaserver.rpcTimeoutMs", &metaOpt->rpcTimeoutMs));
}

void InitSpaceServerOption(Configuration *conf,
    SpaceAllocServerOption *spaceOpt) {
    LOG_IF(FATAL, !conf->GetStringValue(
        "spaceserver.spaceaddr", &spaceOpt->spaceaddr));
    LOG_IF(FATAL, !conf->GetUInt64Value(
        "spaceserver.rpcTimeoutMs", &spaceOpt->rpcTimeoutMs));
}

void InitBlockDeviceOption(Configuration *conf,
    BlockDeviceClientOptions *bdevOpt) {
    LOG_IF(FATAL, !conf->GetStringValue(
        "bdev.confpath", &bdevOpt->configPath));
}
